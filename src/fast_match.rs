...
fn get_num_type(symbols: &[Symbol]) -> NumType {
    ...
    if symbols.len() == 2 {
        if matches!(symbols[1], Sqrt) {
            if matches!(symbols[0], One | Two | Three | Four | Five | Six | Seven | Eight | Nine) {
    ...
    if symbols.len() == 3 && matches!(symbols[2], Div) {
        ...
        if matches!(symbols[0], One | Two | Three | Four | Five | Six | Seven | Eight | Nine)
            && matches!(symbols[1], One | Two | Three | Four | Five | Six | Seven | Eight | Nine)
        {
            return NumType::Rational;
        }
    }
...
fn get_constant_candidates() -> Vec<FastCandidate> {
    vec![
        // Integers
        FastCandidate { symbols: &[Symbol::One] },
        FastCandidate { symbols: &[Symbol::Two] },
        FastCandidate { symbols: &[Symbol::Three] },
        FastCandidate { symbols: &[Symbol::Four] },
        FastCandidate { symbols: &[Symbol::Five] },
        FastCandidate { symbols: &[Symbol::Six] },
        FastCandidate { symbols: &[Symbol::Seven] },
        FastCandidate { symbols: &[Symbol::Eight] },
        FastCandidate { symbols: &[Symbol::Nine] },
        // Named constants
        FastCandidate { symbols: &[Symbol::Pi] },
        FastCandidate { symbols: &[Symbol::E] },
        FastCandidate { symbols: &[Symbol::Phi] },
        FastCandidate { symbols: &[Symbol::Gamma] },
        FastCandidate { symbols: &[Symbol::Plastic] },
        FastCandidate { symbols: &[Symbol::Apery] },
        FastCandidate { symbols: &[Symbol::Catalan] },
        // Simple rationals (common ones)
        FastCandidate { symbols: &[Symbol::One, Symbol::Two, Symbol::Div] },
        FastCandidate { symbols: &[Symbol::One, Symbol::Three, Symbol::Div] },
        FastCandidate { symbols: &[Symbol::Two, Symbol::Three, Symbol::Div] },
        FastCandidate { symbols: &[Symbol::One, Symbol::Four, Symbol::Div] },
        FastCandidate { symbols: &[Symbol::Three, Symbol::Four, Symbol::Div] },
        // Simple roots
        FastCandidate { symbols: &[Symbol::Two, Symbol::Sqrt] },
        FastCandidate { symbols: &[Symbol::Three, Symbol::Sqrt] },
        FastCandidate { symbols: &[Symbol::Five, Symbol::Sqrt] },
        FastCandidate { symbols: &[Symbol::Six, Symbol::Sqrt] },
        FastCandidate { symbols: &[Symbol::Seven, Symbol::Sqrt] },
        FastCandidate { symbols: &[Symbol::Eight, Symbol::Sqrt] },
        FastCandidate { symbols: &[Symbol::Pi, Symbol::Sqrt] },
        FastCandidate { symbols: &[Symbol::E, Symbol::Sqrt] },
        // Simple logs
        FastCandidate { symbols: &[Symbol::Two, Symbol::Ln] },
        FastCandidate { symbols: &[Symbol::Pi, Symbol::Ln] },
        // e ± small integers
        FastCandidate { symbols: &[Symbol::E, Symbol::One, Symbol::Sub] },
        FastCandidate { symbols: &[Symbol::E, Symbol::One, Symbol::Add] },
        // pi ± small integers
        FastCandidate { symbols: &[Symbol::Pi, Symbol::One, Symbol::Sub] },
        FastCandidate { symbols: &[Symbol::Pi, Symbol::One, Symbol::Add] },
        FastCandidate { symbols: &[Symbol::Pi, Symbol::Two, Symbol::Sub] },
        // Common combinations
        FastCandidate { symbols: &[Symbol::One, Symbol::Two, Symbol::Add] },
        FastCandidate { symbols: &[Symbol::One, Symbol::Sqrt, Symbol::One, Symbol::Add] },
        FastCandidate { symbols: &[Symbol::Two, Symbol::Sqrt, Symbol::One, Symbol::Add] },
        // phi combinations (golden ratio)
        FastCandidate { symbols: &[Symbol::Phi, Symbol::One, Symbol::Add] },
        FastCandidate { symbols: &[Symbol::Phi, Symbol::Two, Symbol::Add] },
        FastCandidate { symbols: &[Symbol::Phi, Symbol::Square] },
        // Reciprocals of constants
        FastCandidate { symbols: &[Symbol::Pi, Symbol::Recip] },
        FastCandidate { symbols: &[Symbol::E, Symbol::Recip] },
        FastCandidate { symbols: &[Symbol::Phi, Symbol::Recip] },
    ]
}
...
pub fn find_fast_match(
        ...
        for (idx, uc) in user_constants.iter().enumerate() {
            if idx < 16 && (uc.value - target).abs() < EXACT_TOLERANCE {
                if let Some(sym) = Symbol::from_byte(128 + idx as u8) {
                    let symbols = [sym];
                    if !contains_excluded(&symbols, config.excluded_symbols)
                        && uc.num_type >= config.min_num_type
                    {
                        if let Some(m) = make_match(&symbols, target, EXACT_TOLERANCE) {
                            return Some(m);
                        }
                    }
                }
    ...
    for candidate in candidates {
        ...
        if contains_excluded(candidate.symbols, config.excluded_symbols) {
            continue;
        }
...
fn make_match(symbols: &[Symbol], target: f64, error: f64) -> Option<Match> {
    ...
    Some(Match {
        ...
        x_value: target,
        error,
        complexity: 14, // x = constant is complexity 14
    })
}
...
mod tests {
    ...
    fn test_pi_algebraic_only() {
        ...
        let m = find_fast_match(std::f64::consts::PI, &[], &config);
        assert!(m.is_none(), "Should not find pi when only algebraic allowed");
    }
    ...
    fn test_user_constant_match() {
        ...
        let m = find_fast_match(2.71828, &[uc], &default_config());
        assert!(m.is_some());
    }
}
